...
gelden. Om te detecteren of de browser
probeert af te drukken (of een afdrukvoorbeeld weergeeft), kunnen **Media Queries** worden gebruikt. Door deze instructies te gebruiken, kunnen we
...
de "afdrukmodus" (we zullen iets verderop bekijken hoe je de browser in de afdrukmodus kunt zetten).
...
schuifbalk op de pagina staat, net als op de website. Alle andere elementen zijn ook nog steeds zichtbaar, maar
zijn waarschijnlijk niet bruikbaar voor een gebruiker. Let op: we maken hier enkele aannames! Dit is geen gouden regel dat alleen
de inhoud van de pagina zichtbaar zou moeten zijn voor de gebruiker!
...
een pagina weergeeft voor een afdruk of om deze naar PDF te exporteren. Dit signaal wordt vervolgens in de CSS opgevangen met behulp van de
`@media print`
query. De browser ontvangt dan aanvullende instructies om de pagina anders weer te geven. Deze instructies hebben vaak
voorrang boven de instructies voor de ‘normale weergave’. Het resultaat in dit voorbeeld wordt hieronder weergegeven. Eerst de browser in de
...
![Google Chrome - Afdrukdialoogvenster voorbeeld met mediaqueries.png](images/Google%20Chrome%20-%20Afdrukdialoogvenster%20voorbeeld%20met%20mediaqueries.png)
...
de pagina is ingeschakeld, in plaats van direct op de pagina zelf. Bovendien zijn er nu meerdere pagina’s, zoals we op de onderstaande afbeelding kunnen zien.

![Google Chrome - Voorbeeld van het afdrukvenster met mediaquery's 2.png](images/Google%20Chrome%20-%20Voorbeeld%20van%20het%20afdrukvenster%20met%20mediaquery's%202.png)
...
De *selector* is `article section`: dit betekent dat de sectie een onderliggend element van een artikel moet zijn, en niet zomaar een
sectie.
...
`auto` voor `width` en `height`. Wat is hier dan misgegaan?
...
Bekijk [voorbeeld 03](http://localhost/week06/03) waar dit is gecorrigeerd. Nu is de selector hetzelfde voor afdrukken
...
de resultaten te bekijken. Gelukkig erkennen de browserontwikkelaars deze moeilijkheden en hebben ze ons, ontwikkelaars, een handje geholpen. Er is een manier om
...
van de gebruikte browser. Vaak verschijnt er bij een ‘rechtermuisklik’ op de website een menu met de optie ‘Inspecteren’ of
...
consoletabblad in de onderste helft van het scherm (meestal) te openen. 

![Google Chrome - overzicht ontwikkelaarstools.png](images/Google%20Chrome%20-%20overzicht%20ontwikkelaarstools.png)

Vervolgens openen we in de console het tabblad ‘Rendering’ door op de drie gestapelde puntjes naast ‘Console’ in de
